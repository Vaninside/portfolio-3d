...
| `About` | 14 | `about.title`, `about.description1/2`; 4 stat cells inline cross-section strings: `hero.badge`, `education.gpa` = `"GPA: 3.75/4.00"`, `experience.present` = `"Present"`, `education.graduation` = `"2024"`, `about.highlights.projects` = `"Projects Completed"`; CTA labels `hero.ctaContact` / `hero.ctaCv`; and `aboutSkills: Record<"frontend" \| "academic" \| "leadership" \| "problemSolving", { label, desc }>` replacing the dynamic-key lookup |
...
- **Dead sections dropped:** `language.*`, `footer.copyright`, `footer.builtWith`, and per-section extras (`projects.featured`, `projects.period`, `projects.technologies`, `projects.points`, `education.degree`, `education.university`, `education.thesis`, `contact.email`, `contact.phone`, `contact.location`, `contact.locationValue`, `contact.availability`, `contact.linkedin`, `contact.github`, `organization.role`, `organization.period`) are present in `en.json` but never read — confirmed by grep of `t("…")` call sites. These dropped entries are not transcribed (intentional, not an omission). **Exception:** top-level `education.gpa` (`"GPA: 3.75/4.00"`) and `education.graduation` (`"2024"`) are NOT dead — `About.tsx:132` and `:144` read them for stat cells, so they are inlined in `About`. (The `education.items[].gpa` etc. *array* fields are separate and used by `Education.tsx`.)
