    ...
    >
      <div className="mx-auto max-w-5xl">
        ...
        <div className="relative">
            ...
            {experienceItems.map((exp, i) => {
              return (
                ...
                >
                  ...
                  {/* Card */}
                  <div className="relative rounded-2xl bg-card border border-border p-6 md:p-8 hover:shadow-xl hover:border-primary/30 hover:shadow-primary/5 transition-all duration-300">
                    {/* Role badge */}
                    ...
                    <ul className="space-y-3 mb-6" role="list">
                      {exp.points.map((point) => (
                        ...
                        >
                          <CheckCircle className="size-4 text-primary/60 shrink-0 mt-0.5" aria-hidden="true" />
                          <span>{point}</span>
                        </motion.li>
                      ))}
                    </ul>
          ...
          <motion.div
            className="absolute left-8 bottom-0 -translate-y-1/2 size-3 rounded-full bg-primary border-4 border-background"
            initial={{ opacity: 0, scale: 0 }}
            animate={isInView ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 0 }}
            transition={{ delay: experienceItems.length * 0.15, duration: 0.5, ease: springEase }}
          />
        </div>
      </div>
    </section>
