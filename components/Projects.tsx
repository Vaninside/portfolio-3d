...
function ProjectCard({ project, index, isInView }: { project: typeof projects[0]; index: number; isInView: boolean }) {
  ...
  return (
    <motion.div
      ...
      transition={{ delay: index * 0.08 }} // 30-50ms stagger
      style={{
        rotateX,
        rotateY,
      ...
      <Card className="relative h-full flex flex-col bg-card/80 backdrop-blur-sm border-border/50 hover:border-primary/30 hover:shadow-2xl transition-all duration-300 overflow-hidden">
        ...
        <CardHeader className="pb-4">
          ...
          <motion.div variants={contentVariants} className="flex items-center gap-2 mt-1">
            <project.icon className="size-5" style={{ color: project.color.split(" ")[0].replace("from-", "") }} aria-hidden="true" />
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-5xl">
        ...
        {/* Projects grid */}
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-2" role="list" aria-label="Projects">
